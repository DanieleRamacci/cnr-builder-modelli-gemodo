...
| `010-configurazione-cataloghi-integrazioni` | Interfaccia di amministrazione per definire tipi documento/tipologie/profili/campi, generare il contratto di discovery per un sistema esterno e registrarne l'endpoint | Draft (2026-09-15), spec.md scritta, `plan.md`/`data-model.md`/`tasks.md` non ancora generati | n/d (nata da `docs/adr/0001-ownership-dati-esterni-e-onboarding-contesti.md`, non dalla `PROPOSTA` originale) | Nuova spec nata dal cambio di paradigma su ownership dei dati esterni (referenziare non copiare, `DEC-001-OWNERSHIP-DATI-ESTERNI`); sostituisce, per questo incremento, il percorso file+deploy previsto da `DEC-001-CONFIG-PROFILO-GEBAN`. Primo caso d'uso reale: GEBAN/`BANDO_CONCORSO`, esempio in `docs/adr/0001-esempio-discovery-geban.json` |
