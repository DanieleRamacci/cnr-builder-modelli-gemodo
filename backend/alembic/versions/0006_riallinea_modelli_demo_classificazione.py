"""riallinea modelli demo con classificazione bandi
...
def downgrade() -> None:
    ...
    for item in catalogo.get("modelli", []):
        codice_modello = item["modello_versione_id"]
        if codice_modello in {
            "demo-bando-concorso-standard-v1",
            "demo-bando-concorso-html-libero-non-valido",
        }:
            continue
        bind.execute(sa.text("DELETE FROM modello_documento WHERE id = :id"), {"id": _uuid(f"modello:{codice_modello}")})
