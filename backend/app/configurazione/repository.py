...
from app.configurazione.models import DefinizioneStruttura, EndpointIntegrazione, SchemaDiscoveryGenerato
...
def endpoint(db: Session, integrazione_id):
    if integrazione_id is None:
        return None
    return db.scalar(select(EndpointIntegrazione).where(EndpointIntegrazione.integrazione_id == integrazione_id))
