...
- Q: Quale linea Keycloak proponiamo per utenti GEMODO e chiamate GEBAN? -> A: Per utenti GEMODO si usa login SSO Keycloak con client/ruoli GEMODO. Per GEBAN si usa chiamata backend-to-backend: GEBAN autorizza l'utente sul bando, poi `geban-backend` chiama `gemodo-backend` con token tecnico (client credentials); utente reale e contesto bando passano nel payload per audit, tracciabilita' e idempotenza. Questa linea e' stata confermata come decisione definitiva `SEC-006-001` nella spec `006` il 2026-07-29; resta da configurare concretamente lato Keycloak (client, audience, ruoli), self-service in ambiente di test (`sso.test.si.cnr.it`, realm `cnr`), tramite richiesta al referente infrastruttura per la produzione.
...
- Chiamata GEBAN-GEMODO implementata con token exchange o identita' utente nel JWT, invece del token tecnico deciso in `SEC-006-001`.
...
- **FR-031**: Le fondamenta sicurezza MUST restare configurabili tramite variabili d'ambiente (issuer URL, client id/secret, audience) finche' la configurazione Keycloak di produzione non e' disponibile; in locale/test devono poter puntare sia a un principal mock sia al realm Keycloak CNR di test reale (`sso.test.si.cnr.it`, realm `cnr`), senza hardcodare token exchange (non richiesto, vedi `SEC-006-001`) o nomi client definitivi.
...
- **Regole di sicurezza per API catalogo e generazione**: owner `006`; decisione confermata (`SEC-006-001`, 2026-07-29): utenti GEMODO con login SSO e ruoli GEMODO; chiamate operative GEBAN tramite token tecnico `geban-backend` (client credentials) verso audience/client API GEMODO, con utente reale e contesto bando nel payload per audit. Token exchange/on-behalf-of resta variante futura non richiesta per la prima release.
...
- **Gestione lingua italiano/inglese del bando**: owner `001` e `004` con supporto `003`.
  Aggiornamento 2026-09-21: la precedente ipotesi del flag `Bando Inglese` e della
  doppia generazione implicita e' superseduta da `DEC-001-LINGUA-IT-EN`. Italiano
  e inglese sono modelli separati; catalogo e generazione usano una
  `modello_versione_id` per documento.
...
- **Client Keycloak GEMODO e GEBAN**: owner `006` con supporto `009`; decisione confermata: due client separati `gemodo-frontend`/`gemodo-backend`, piu' `geban-backend` (e placeholder `geban-frontend` se non ancora gestito da GEBAN) creati come client di test dal team GEMODO nel realm Keycloak CNR di test `cnr` (`sso.test.si.cnr.it`), con accesso amministratore concesso al team. La produzione replica la stessa configurazione tramite richiesta al referente infrastruttura Keycloak CNR. Resta da verificare se `geban-frontend`/`geban-backend` esistono gia' nel realm di test a cura del team GEBAN. Fase bloccante superata per Phase 1/2 della `009`; resta da eseguire la configurazione concreta in Keycloak.
